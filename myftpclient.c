...
int main(int argc, char *argv[]) {
  ...
  // mysr_sender
  struct mysr_sender sender; 

  // init mysr_send
  mysr_init_sender(&sender, ip, port, N, timeout);
  ...
  mysr_send(&sender, (unsigned char*)&filenamesize, sizeof(filenamesize));
  filenamesize = ntohl(filenamesize);
  fprintf(stdout, "APP::send filename size = %d\n", filenamesize);
  
  // (2) send file name 
  mysr_send(&sender, (unsigned char*)filename, filenamesize); 
  fprintf(stdout, "APP::send filename = %s\n", filename);
  
  // (3) send file size 
  filesize = htonl(filesize);
  mysr_send(&sender, (unsigned char*)&filesize, sizeof(filesize));
  filesize = ntohl(filesize); 
  fprintf(stdout, "APP::send filesize = %d\n", filesize);
  
  // (4) send actual file
  mysr_send(&sender, data, filesize); 
