...
int main(int argc, char** argv){
  ...
  // mysr_receiver
  struct mysr_receiver receiver;

  // init mysr_receiver
  mysr_init_receiver(&receiver, port);

  while(1) {
    // (1) read filename size
    int filenamesize;
    mysr_recv(&receiver, (unsigned char*)&filenamesize, sizeof(filenamesize));
    ...
    mysr_recv(&receiver, filename, filenamesize);
    printf("APP::filename = %s\n", filename);

    // (3) read file size
    int filesize;
    mysr_recv(&receiver, (unsigned char*)&filesize, sizeof(filesize));
    ...
    while (f_left > 0) {
      ...
      recv_size = mysr_recv(&receiver, (unsigned char*)buff, sizeof(buff));
      f_left -= recv_size;
